...
First, install a Python environment and then install the latest Dr.TVAM version from PyPI:
...
- ``sensor``: describes the volumetric voxel grid used for the optimization. The sensor is a DDA (Digital Differential Analyzer) type, which is a method for ray tracing in a voxel grid. The voxel grid has a scale of 4 in all dimensions, meaning each voxel represents a 4mm x 4mm x 4mm volume. The film is a volumetric film (vfilm) with a resolution of 200x200x200 voxels, which means the total volume covered by the sensor is 800mm x 800mm x 800mm. The voxel size should be equal to the pixel size of the projector to ensure that the projected patterns are properly sampled by the sensor.
...
- ``loss``: describes the loss function used for optimization. The loss is of type "threshold", which means it will penalize the difference between the predicted and target transmissions based on two thresholds: tl (lower threshold) and tu (upper threshold). If the predicted transmission is below tl, it will be penalized as a false negative; if it is above tu, it will be penalized as a false positive. The values of tl and tu are set to 0.80 and 0.95, respectively.
...
- target.exr: the theoretical target slices corresponding to the target shape. This is what we want to achieve with the optimization.
...
Here is an example of how the final dose looks like in the tev viewer. Each small square is a slice of the volume:

.. image:: resources/tev_final.png
  :width: 600


We can also zoom in to see the details of the dose distribution: 

.. image:: resources/tev_final_zoomed.png
  :width: 600


And we can also load the patterns in the tev viewer to see how they look like:
